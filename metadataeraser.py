...
def cleaned_dirs(path, dirs, subdirs, root):
    r_path = '\/[\w ]*'
...
def path_cleaned_dir(origin_file, root, path):
    ...
    for name in dir_list:
        name = name + '.cleaned'
        cleaned_path +=  name
    final_cleaned_path = get_parent_dir(path)[0] + cleaned_path + '/' + origin_file
...
def cleaned_file(file):
    ...
    name1 = file[:x]
    name2 = file[x:]
    cleaned_file = name1 + '.cleaned' + name2
    return cleaned_file
...
def cleaned_files(root, files, path):
    ...
    for file in files:
        ...
        if output == b'':
            ...
            shutil.move((root + '/' +cleaned_file(file)), path_cleaned_dir(cleaned_file(file), root, path))
        else:
            #create not_cleaned.txt add metadata
            os.system('echo ' + str(output) + ' >> not_cleaned.txt')
            os.system('echo ' + str(metadata) + ' >> not_cleaned.txt')
            os.system('echo location.cleaned: ' + str(path_cleaned_dir(file, root, path)) + ' >> not_cleaned.txt')
...
def main():
    ...
    for root, dirs, subdirs in os.walk(path):
        cleaned_dirs(path, dirs, subdirs, root)
    for root, dirs, files in os.walk(path):
            cleaned_files(root, files, path)
